...
phe="/home/liuqb/workspace/gwas-gemma-resequencing/OD_含量_3st.txt"
...
do
  ...
  cut -f1 ${i}.assoc.txt|sed 1d >chr.txt
  cut -f3 ${i}.assoc.txt|sed 1d >ps.txt
  paste snp.txt chr.txt ps.txt >tmp.txt
  rm snp.txt chr.txt ps.txt
  # 替换染色体表示方式：A01->1,若染色体已替换，可注释下行
  ...
  # 合并 p value，添加表头，sed 中使用变量，需要用双引号;不同版本的gemma产生的P值列不同，0.98版在第10列 
  cut -f10 ${i}.assoc.txt |sed 1d|paste tmp.txt -|sed "1i snp\tchr\tps\t${prefix}" - >${i}_${prefix}.cmplot.txt && rm tmp.txt
done
